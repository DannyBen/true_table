describe TrueTable do
  ...
  let(:data) do
    [
      { year: 2020, population: 2_000_000 },
      { year: 2021, population: 20_000 },
  ...
  describe 'class methods' do
    ...
    describe '::from_csv' do
      ...
      it 'returns a TrueTable' do
        expect(subject.from_csv csv).to be_a described_class
      end

      it 'loads the CSV into the table' do
        expect(subject.from_csv(csv)[1]).to eq({ population: 20_000, year: 2021 })
      end
    end
    ...
    describe '::load_csv' do
      ...
      it 'returns a TrueTable' do
        expect(subject.load_csv path).to be_a described_class
      end

      it 'loads the CSV into the table' do
        expect(subject.load_csv(path)[1]).to eq({ population: 20_000, year: 2021 })
      end
    end
  end
  ...
  describe '#[]=' do
    context 'with an integer key' do
      it 'adds or updates the row' do
        ...
        expect(subject[:population]).to eq [2_000_000, 20_000, 200, 2, 3]
      end
    end
  ...
  describe '#clone' do
    it 'returns a deep copy without altering the original' do
      copy = subject.clone
      copy[:cured] = ['10%', '5%', '3%', '1%']
      expect(subject.headers).to eq %i[year population]
      expect(copy.headers).to eq %i[year population cured]
    end
  end
  ...
  describe '#combination' do
    it 'yields the row combinations' do
      expect(subject.combination(2).to_a.size).to eq 6
    end
  end
  ...
  describe '#delete' do
    it 'deletes rows for which the block returns true' do
      subject.delete({ year: 2021, population: 20_000 })
      expect(subject.count).to eq 3
    end
  end
  ...
  describe '#delete_at' do
    context 'with an integer argument' do
      it 'deletes a row in place and returns the deleted row' do
        expect(subject.delete_at 1).to eq({ population: 20_000, year: 2021 })
        expect(subject.count).to eq 3
      end
    end
  ...
  describe '#drop' do
    ...
    it 'returns a Table object' do
      expect(subject.drop(2)).to be_a described_class
    end
  end
  ...
  describe '#fetch' do
    context 'with an integer argument' do
      it 'returns a row' do
        expect(subject.fetch 1).to eq({ year: 2021, population: 20_000 })
      end
  ...
  describe '#filter' do
    it 'returns a new table with matching rows' do
      result = subject.filter { |row| row[:population] < 20_000 }
  ...
  describe '#filter!' do
    it 'keeps only selected rows' do
      subject.filter! { |row| row[:population] < 20_000 }
  ...
  describe '#include?' do
    it 'returns true of the row exists' do
      expect(subject.include? year: 2020, population: 2_000_000).to be true
    end
  end
  ...
  describe '#max' do
    it 'returns the row with the maximum block value' do
      result = subject.max_by { |a| a[:population] }
  ...
  describe '#min' do
    it 'returns the row with the minimum block value' do
      result = subject.min_by { |a| a[:population] }
  ...
  describe '#prepend' do
    it 'inserts a row at the beginning' do
      subject.prepend({ year: 2019, population: 3_000_000 })
  ...
  describe '#reject' do
    it 'returns a new table without matching rows' do
      result = subject.reject { |row| row[:population] < 20_000 }
  ...
  describe '#reject!' do
    it 'keeps only non-rejected rows' do
      subject.reject! { |row| row[:population] < 20_000 }

      expect(subject[:population]).to eq [2_000_000, 20_000]
    end
  end
  ...
  describe '#replace' do
    it 'replaces the table with a new table' do
      ...
      expect(subject[:infected][1]).to eq 1_980_000
  ...
  describe '#rotate' do
    it 'offsets the rows to the top' do
      ...
      expect(subject.rotate(2)).to be_a described_class
    end
  end
  ...
  describe '#select!' do
    it 'keeps only selected rows' do
      subject.select! { |row| row[:population] < 20_000 }
  ...
  describe '#shuffle' do
    it 'returns a new table with shuffled rows' do
      expect(subject.shuffle).to be_a described_class
    end
  end
  ...
  describe '#sort' do
    it 'returns a new sorted array' do
      result = subject.sort_by { |a| a[:population] }

      expect(result).to be_a described_class
      expect(result[0][:population]).to eq 2
      expect(result[3][:population]).to eq 2_000_000
      expect(subject[0][:population]).to eq 2_000_000
    end
  end
  ...
  describe '#to_a' do
    it 'returns an Array' do
      ...
      expect(subject.to_a).not_to be_a described_class
    end
  end

  describe '#to_ary' do
    it 'returns self' do
      expect(subject.to_ary).to be_a described_class
    end
  end
  ...
  describe '#values' do
    it 'returns an array of arrays' do
      ...
      expect(result.first).to eq [2020, 2_000_000]
    end
  end
